...
export async function analisarPDFComOpenRouter(params: AnalisePDFParams): Promise<ResultadoAnalise> {
  ...
  try {
    ...
    if (!process.env.OPENROUTER_API_KEY) {
      console.error("Erro crítico: OPENROUTER_API_KEY não está definida no ambiente")
      throw new Error("Erro de configuração do servidor. Por favor, entre em contato com o suporte.")
    }

    // OpenRouter aceita apenas imagens, então precisamos converter o PDF para imagens
    console.log("Convertendo PDF para imagens para uso com OpenRouter")
    const editaImages = await convertPDFToImages(fileContent)
    
    // Se tiver uma matrícula também, vamos converter
    let matriculaImages: string[] = []
    if (fileMatriculaContent) {
      console.log("Convertendo matrícula para imagens")
      matriculaImages = await convertPDFToImages(fileMatriculaContent)
    }
    
    // Combinar todas as imagens em um array 
    const allImages = [...editaImages, ...matriculaImages]
    
    // Criar a parte de conteúdo da mensagem com texto e imagens
    const content: any[] = [
      {
        type: "text",
        text: prompt,
    ...
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      headers: {
        ...
        "HTTP-Referer": "https://analiseleilao.vercel.app",
        "X-Title": "AnaliseLeilão",
      },
      body: JSON.stringify({
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        throw new Error("Erro de autenticação na API. Verifique as credenciais.")
    ...
    console.error("Erro ao analisar PDF com OpenRouter:", error)
    
    // Classificar os erros para mensagens mais informativas
    ...
    // Retornar um resultado com mensagem de erro mais específica
    return {
      id,
...
// Função principal que será chamada pelo actions.ts
export async function analisarPDF(params: AnalisePDFParams): Promise<ResultadoAnalise> {
  // Esta função agora apenas direciona para a função específica do OpenRouter
